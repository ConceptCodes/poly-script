...
class TestTeamService:
    def test_create_team_success(self, mock_db, team_service):
        ...
        with (
            patch.object(team_service, "team_repo", mock_team_repo),
            patch.object(team_service, "team_member_repo", mock_team_member_repo),
        ):
    ...
    def test_create_team_too_many_teams(self, team_service):
        mock_user = Mock()
        mock_user.id = "user-id"
    ...
    def test_add_member_success(self, mock_db, team_service):
        ...
        with (
            patch.object(team_service, "team_repo", mock_team_repo),
            patch.object(team_service, "user_repo", mock_user_repo),
            patch.object(team_service, "team_member_repo", mock_team_member_repo),
        ):
