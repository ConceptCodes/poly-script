...
from ..constants import PLAN_LIMITS, JobState, PlanType
...
class ActivityRegistry:
    ...
    @classmethod
    def get_event_type_info(cls, event_type: str) -> Optional[Dict[str, str]]:
...
class DashboardService:
    ...
    def get_dashboard_stats(self, team_id: uuid.UUID, user_id: uuid.UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_members": plan_limits["members"] if plan_limits["members"] != float("inf") else None,
            "max_jobs_per_month": plan_limits["uploads_per_month"] if plan_limits["uploads_per_month"] != float("inf") else None,
        }
    
    def get_recent_jobs(self, team_id: uuid.UUID, user_id: uuid.UUID, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    def get_team_activity(self, team_id: uuid.UUID, user_id: uuid.UUID, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        for job in jobs:
            if job.state == JobState.SUCCEEDED:
                activities.append({
                    "type": "job_completed",
                    "display_name": ActivityRegistry.EVENT_TYPES["job_completed"]["display_name"],
    ...
    def _get_team_access(self, team_id: uuid.UUID, user_id: uuid.UUID) -> Optional[Team]:
