...
class TranscriptService:
    ...
    def revert_to_original(
        ...
        if edits:
            ...
            original_segments = edits[-1].previous_segments if edits[-1].previous_segments else transcript.segments
        else:
    ...
    def list_transcripts(
        ...
        total = len(self.session.execute(count_stmt).all())
        
        # Apply pagination
        stmt = stmt.order_by(Transcript.created_at.desc())
        stmt = stmt.offset((page - 1) * page_size).limit(page_size)
        
        transcripts = self.session.execute(stmt).scalars().all()
