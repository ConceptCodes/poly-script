...
class TranscriptEditRepository(BaseRepository[TranscriptEdit]):
    ...
    def get_history_by_transcript_id(self, transcript_id: uuid.UUID) -> List[TranscriptEdit]:
        stmt = select(TranscriptEdit).where(TranscriptEdit.transcript_id == transcript_id).order_by(TranscriptEdit.created_at.desc())
        return self.session.execute(stmt).scalars().all()

    def list_by_user_id(self, user_id: uuid.UUID) -> List[TranscriptEdit]:
        stmt = select(TranscriptEdit).where(TranscriptEdit.user_id == user_id).order_by(TranscriptEdit.created_at.desc())
        return self.session.execute(stmt).scalars().all()
