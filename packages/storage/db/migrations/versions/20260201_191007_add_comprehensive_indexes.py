...
down_revision: str | Sequence[str] | None = "20260201_174324"
...
def upgrade() -> None:  # noqa: PLR0915
    ...
    op.create_index(
        "ix_transcription_jobs_team_status", "transcription_jobs", ["team_id", "status"]
    )
    ...
    op.create_index(
        "ix_translation_artifacts_target_language", "translation_artifacts", ["target_language"]
    )
    ...
    op.create_index(
        "ix_subscriptions_current_period_start", "subscriptions", ["current_period_start"]
    )
    ...
    op.create_index(
        "ix_usage_logs_team_action_created", "usage_logs", ["team_id", "action", "created_at"]
    )
