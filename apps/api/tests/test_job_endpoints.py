...
def isolate_app_dependencies():
    """Keep endpoint tests isolated from real auth and database dependencies."""
    original_overrides = app.dependency_overrides.copy()
    with patch("src.routes.jobs.get_db_session") as mock_get_session:
...
def override_team(team_id: uuid.UUID) -> None:
    """Override the route dependency FastAPI registered for team auth."""
...
class TestJobListEndpoint:
    ...
    async def test_list_jobs_empty(self, client: AsyncClient, sample_team_id):
        ...
        override_team(sample_team_id)

        with patch("src.routes.jobs.TranscriptionJobRepository") as mock_repo_class:
            mock_repo = Mock()
            mock_repo_class.return_value = mock_repo
            mock_repo.get_by_team_id.return_value = []

            with patch("src.routes.jobs.AudioAssetRepository") as mock_audio_repo_class:
    ...
    async def test_get_job_success(self, client: AsyncClient, sample_team_id, sample_job_id):
        ...
        with patch("src.routes.jobs.TranscriptionJobRepository") as mock_repo_class:
            ...
            with patch("src.routes.jobs.AudioAssetRepository") as mock_audio_repo_class:
                ...
                mock_audio = Mock()
                mock_audio.filename = "test.mp3"
                mock_audio.duration_seconds = 12.5
                mock_audio_repo.get_by_job_id.return_value = mock_audio
